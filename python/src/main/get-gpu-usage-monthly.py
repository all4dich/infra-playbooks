...
arg_parser.add_argument("--pivot-columns")
...
if __name__ == "__main__":
    ...
    pivot = df.pivot_table(index=target_index, columns=target_columns, values=target_values, aggfunc=lambda x: numpy.float64(sum(x)))
    ...
    slot_number = (end - start).total_seconds() / 300
    ...
    for index_a in pivot.index:
        ...
        for is_used in value.index:
            # print(is_used, hostname, value[is_used, hostname], slot_data[hostname.lower()])
            used_value = value[is_used]
            slot_value = slot_data
            if str(used_value) != str(np.nan):
                new_value = 100 * (used_value / slot_value)
                print(used_value, slot_value, new_value)
                pivot_percent.loc[index_a].loc[is_used] = int(new_value)
                logging.info("Updated")

    logging.info("Write a pivot table to " + pivot_usage_percent_file_path)
    pivot_percent.to_excel(pivot_usage_percent_file_path)
